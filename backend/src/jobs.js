 ...
 *
 * A customer change (added, edited, paid, paused, moved, deleted) already brings its
 * router in line within seconds (queueRouterSync in router-queues.js). This pass is
 * the safety net for everything that does not go through one of those: a customer
 * who expired overnight, a queue edited on the router by hand, a router that was
 * unreachable when the change happened.
 *
 * What it does not do is wipe: only Refresh clears the list and writes it fresh.
...
async function syncRouterQueuesJob() {
  ...
  try {
    const { syncRouterNow } = await import('./router-queues.js');
    const { rows } = await pool.query(
    ...
    for (const r of rows) {
      try {
        const out = await syncRouterNow(r);
