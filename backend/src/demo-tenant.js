...
import { seedTenant } from './seed-tenant.js';

/**
 * Wipe and reseed the sales-demo tenant ("demo" subdomain) — shared by the
 * one-off script (scripts/seed-demo-tenant.mjs) and jobs.js's own hourly
 * cron. Whatever a prospect has been clicking around and changing gets
 * thrown away on a fixed schedule, without the demo needing to be a
 * separate, unreal, backend-less build of the app — it is the real app,
 * hitting the real API, on data that simply never gets to stick around.
 *
 ...
 *     is actually offline.
 *
 * Does not call pool.end() — this runs inside the long-lived api process
 * (both from the cron job and, transitively, from the CLI script), so the
 * shared pool has to outlive a single call.
 */
...
export async function resetDemoTenant(password) {
  const demoPassword = password ?? process.env.DEMO_PASSWORD ?? 'DemoFiber2026!';
  const c0 = await pool.connect();
  try {
    await c0.query('begin');

    let { rows: [tenant] } = await c0.query('select * from tenants where subdomain=$1', [DEMO_SUBDOMAIN]);
    if (!tenant) {
    ...
    // Wipe this tenant's own rows in everything this seeds, so every reset
    // gives a clean slate rather than piling up duplicates.
    ...
    // payroll_items has no tenant_id of its own — scoped via run_id, and
    // payroll_runs above already cascades into it.
    ...
    // Only now, not earlier: seedTenant runs on the pool's own separate
    // connection, so it cannot see the tenant (or anything else above)
    // until c0's own transaction has actually committed.
    ...
    return { tenantId: tid, password: demoPassword };
  } catch (e) {
