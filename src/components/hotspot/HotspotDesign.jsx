  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Portal Design</h3>
          <p className="text-sm text-slate-500">Customize the look and feel of your captive portal</p>
        </div>
        <div className="flex bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
          <button
            onClick={() => setActiveView('desktop')}
            className={`p-2 rounded-lg transition-all ${activeView === 'desktop' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Monitor className="w-5 h-5" />
          </button>
          <button
            onClick={() => setActiveView('mobile')}
            className={`p-2 rounded-lg transition-all ${activeView === 'mobile' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <Card className="border-none shadow-md">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <label className="text-xs font-bold text-slate-500">Selected Theme</label>
                <div className="grid grid-cols-2 gap-2">
                  {themes.map((theme) => (
                    <button
                      key={theme.id}
                      onClick={() => setSelectedTheme(theme.id)}
                      className={`relative flex flex-col gap-2 p-3 rounded-xl border-2 transition-all text-left ${selectedTheme === theme.id ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-200 bg-slate-50'}`}
                    >
              ...
              <div className="divide-y divide-slate-100">
                <button className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-700">Branding & Logo</span>
                  </div>
                ...
                <button className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-700">Messaging & Text</span>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300" />
                </button>
                <button className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-700">Page Structure</span>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300" />
                </button>
              </div>
        ...
        {/* Preview area */}
        <div className="lg:col-span-8 flex flex-col items-center justify-center p-8 bg-slate-100 rounded-2xl border-2 border-dashed border-slate-200">
          <motion.div
            layout
            className={`bg-white shadow-2xl overflow-hidden rounded-3xl border-8 border-slate-200 transition-all ${activeView === 'mobile' ? 'w-[320px] aspect-[9/18]' : 'w-full aspect-video'}`}
          >
            {/* Mock Portal Header */}
          ...
          <div className="mt-4 flex items-center gap-2 text-slate-400 bg-white px-4 py-2 rounded-full border border-slate-200 shadow-sm">
...
function Button({ children, className = '', ...props }) {
  return (
    <button className={`inline-flex items-center justify-center rounded-xl font-bold transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 text-white bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-200 ring-indigo-500 ${className}`} {...props}>
      {children}
    </button>
  );
}
