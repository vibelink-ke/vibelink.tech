...
export default function HotspotVouchers() {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'available': return { color: 'text-emerald-600 bg-emerald-50', icon: CheckCircle };
      case 'used': return { color: 'text-blue-600 bg-blue-50', icon: Clock };
      case 'expired': return { color: 'text-rose-600 bg-rose-50', icon: XCircle };
      default: return { color: 'text-slate-600 bg-slate-50', icon: CheckCircle };
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Voucher Management</h3>
          <p className="text-sm text-slate-500">Generate, track and manage access vouchers</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-none shadow-sm bg-gradient-to-br from-indigo-500 to-indigo-600 text-white">
          <CardContent className="p-6">
            ...
            <div className="mt-4 flex items-center gap-1.5 text-xs font-semibold bg-white/20 w-fit px-2 py-1 rounded-full">
        ...
        <Card className="border-none shadow-sm bg-white">
          <CardContent className="p-6">
            <p className="text-sm font-medium text-slate-500">Redeemed This Month</p>
            <h4 className="text-3xl font-bold mt-1">856</h4>
        ...
        <Card className="border-none shadow-sm bg-white border border-slate-100">
          <CardContent className="p-6">
            <p className="text-sm font-medium text-slate-500">Expired Vouchers</p>
            <h4 className="text-3xl font-bold mt-1 text-rose-500">142</h4>
            <div className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-rose-600 bg-rose-50 w-fit px-2 py-1 rounded-full">
      ...
      <Card className="border-none shadow-md overflow-hidden bg-white">
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-50/50">
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none border transition-all"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <button className="flex-1 sm:flex-none px-4 py-2 text-sm font-medium text-slate-600 hover:bg-white border-slate-200 border rounded-xl transition-all shadow-sm">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 italic">
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Service Plan</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Status</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Created Date</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Expiry</th>
              ...
              {mockVouchers.map((v, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 font-medium">{v.plan}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">{v.created}</td>
                    <td className="px-6 py-4 text-sm text-slate-500">{v.expiry}</td>
                    <td className="px-6 py-4">
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-sm text-slate-500">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1 bg-white border-slate-200 border rounded-lg hover:bg-slate-50 disabled:opacity-50" disabled>Previous</button>
            <button className="px-3 py-1 bg-white border-slate-200 border rounded-lg hover:bg-slate-50">Next</button>
          </div>
        </div>
      </Card>
    </div>
