...
export default function MessagingPanel({ customer, user }) {
  ...
  useEffect(() => {
    const unreadStaffMessages = messages.filter(
      m => m.sender_type === 'staff' && !m.read
    );
    unreadStaffMessages.forEach(m => {
  ...
  const unreadCount = messages.filter(m => m.sender_type === 'staff' && !m.read).length;
            ...
            <AnimatePresence>
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-sm text-slate-400 mt-1">Start a conversation with support</p>
                </div>
              ) : (
                messages.map((msg, i) => (
                  <motion.div
                    ...
                    className={`flex gap-3 ${msg.sender_type === 'customer' ? 'justify-end' : 'justify-start'}`}
                  >
                    ...
                    <div className={`max-w-[70%] ${msg.sender_type === 'customer' ? 'items-end' : 'items-start'}`}>
